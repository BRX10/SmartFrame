...
Texte noir pur. Icone play + tags + hook dans le bandeau.
"""
...
from resources.music_masks._common import (
    ...
    format_tags, draw_icon_play, draw_icon_heart
)
...
def _render(title, artist, album, artwork, width, height, enrichment=None):
    ...
    if hook:
        band_lines += 1
    band_h = max(90, PADDING * 2 + band_lines * 34)
    ...
    text_max_w = width - 2 * PADDING - 28  # -28 pour icone play

    ty = band_y + PADDING // 2

    # Icone play en debut de bandeau
    draw_icon_play(draw, PADDING + 6, ty + 14, size=12)
    content_x = PADDING + 28

    # Titre (2 lignes max)
    title_lines = wrap_text(draw, title or "Titre inconnu", ft, text_max_w, max_lines=2)
    ty = draw_text_block(draw, title_lines, ft, content_x, ty, fill=(0, 0, 0), line_height=44)
    ty += 4

    # Artiste
    draw.text((content_x, ty), truncate_text(draw, artist or "Artiste inconnu", fa, text_max_w),
              fill=(0, 0, 0), font=fa)
    ty += 30

    # Album + annee
    if album_year:
        draw.text((content_x, ty), truncate_text(draw, album_year, fb, text_max_w),
                  fill=(0, 0, 0), font=fb)
        ty += 24

    # Tags
    if tags:
        draw.text((content_x, ty), truncate_text(draw, tags, ftags, text_max_w),
    ...
    if hook:
        ty += 4
        draw.text((content_x, ty), truncate_text(draw, f"« {hook} »", fh, text_max_w),
                  fill=(0, 0, 0), font=fh)

    # Coeur en bas a droite du bandeau
    draw_icon_heart(draw, width - PADDING - 8, height - PADDING - 8, size=12)
